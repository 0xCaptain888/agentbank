...
export function analystCommands(program: Command) {
  ...
  analyst
    ...
    .action(async (opts, cmd) => {
      ...
      if (!opts.confirm) {
        return writeOutput(fmt, {
          dry_run: true,
          stake_mnt: stake,
          domain: opts.domain,
          min_stake: 100,
          unstake_delay_days: 7,
        });
      }
      ...
      writeOutput(fmt, {
        ...
        stake_mnt: stake,
        domain: opts.domain,
        message: 'Analyst registered. You can now post signals.',
      });
    });

  analyst
    .command('earnings')
    .description('View accrued analyst fees')
    .action(async (opts, cmd) => {
      const fmt = cmd.parent!.parent!.opts().output;
      writeOutput(fmt, {
        unclaimed_usdc: '0',
  ...
  analyst
    ...
    .action(async (opts, cmd) => {
      ...
      if (!opts.confirm) {
        return writeOutput(fmt, {
          dry_run: true,
          message: 'Will request unstake. 7-day delay applies.',
        });
      }
      writeOutput(fmt, { status: 'unstake_requested', delay_days: 7 });
    });

  analyst
    .command('list')
    .description('List all active analysts')
    .action(async (opts, cmd) => {
      const fmt = cmd.parent!.parent!.opts().output;
      writeOutput(fmt, {
        analysts: [
